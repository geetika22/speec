...
const VicePresidentCard = ({ img, name, _id, dept, batch }) => {
  ...
  const postVote = () => {
    axios
      .post("http://localhost:5000/vicepresidentvote", vote)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(
        "http://localhost:5000/vicepresidentvoted/" +
      ...
      <NomineeCardContainer>
        ...
        {data["voted"] === false ? (
          ...
          <Button
            className="button"
            onClick={() => {
              alert("You already voted to certain Vice President!!!");
            }}
          >
            Voted
          </Button>
