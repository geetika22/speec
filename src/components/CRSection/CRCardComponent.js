...
const CRCard = ({ img, name, _id, dept, batch }) => {
  ...
  const postVote = () => {
    axios
      .post("http://localhost:5000/crvote", vote)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/crvoted/" + localStorage.getItem("rollno"))
      .then((res) => {
        setdata(res.data);
      })
      .catch((err) => {
      ...
      <NomineeCardContainer>
        ...
        {data["voted"] === false ? (
          ...
          <Button
            className="button"
            onClick={() => {
              alert("You already voted to certain Class Representative!!!");
            }}
          >
            Voted
          </Button>
